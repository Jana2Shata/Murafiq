...
translations = {
    'en': {
        'page_title': "Murafiq | For Help and Guidance",
        'banner_title': "🕋 Murafiq – For Help and Guidance",
        'welcome_text': "Welcome to the Murafiq Assistant.",
        'input_placeholder': "📝 Ask your question below:",
        'submit_button': "🔍 Submit",
        'response_title': "### 💡 Murafiq Assistant Response:",
        'empty_input_warning': "⚠️ Please enter a valid question.",
        'language_button': "🌐 العربية"
    },
    'ar': {
        'page_title': "للعون والإرشاد رافق | مرافق",
        'banner_title': "🕋 مرافق - للعون والإرشاد",
        'welcome_text': " مرحبًا بك في مساعد مرافق",   
...
st.markdown("""
    ...
    .stApp {
        ...
        background-image: linear-gradient(to bottom right, #0a1128, #4d5c21); /* Gradient background */
    }
...
with col1:
    murafiq_logo_path = os.path.join(os.path.dirname(__file__), 'static', 'murafiq logo.png')
    st.image(murafiq_logo_path, width=90, caption="", clamp=True, output_format="auto")
...
if prompt := st.chat_input(get_text('input_placeholder')):
    ...
    # Get response from the vector store
    with st.chat_message("assistant"):
        ...
        try:
            ...
            relevant_docs = get_relative(prompt)
            
            if relevant_docs:
                full_response = "Here is the most relevant question I found to yours:\n\n" + \
                                relevant_docs[0].page_content + "\n\n" + \
                                "Please note that this is only a temporary answer, please await " + \
                                "for the final response from the official Sheikhs."
            else:
                full_response = "I couldn't find a relevant answer to your question."
